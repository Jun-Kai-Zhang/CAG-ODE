...
class VAE_Baseline(nn.Module):
	...
	def compute_all_losses(self, batch_dict_encoder, batch_dict_decoder, batch_dict_graph, num_atoms, edge_lamda,
							   alpha, beta, kl_coef=1., istest=False):
		'''
		...
		pred_node, pred_edge, pred_treatment_rs, pred_interference_rs, info, temporal_weights = self.get_reconstruction(
			batch_dict_encoder, batch_dict_decoder, num_atoms=num_atoms)
		...
		treatment_balancing_loss = self.bce_loss(pred_treatment_rs, batch_dict_decoder["treatment"])
		# rec_likelihood_interfere = self.get_gaussian_likelihood(
		interference_balancing_loss = self.mse_loss(pred_interference_rs, batch_dict_decoder["interference"])

		rec_likelihood = (1 - edge_lamda) * rec_likelihood_node + edge_lamda * rec_likelihood_edge - alpha * treatment_balancing_loss - beta * interference_balancing_loss
		...
		results["likelihood_node"] = torch.mean(rec_likelihood_node).data.item()
		results["likelihood_edge"] = torch.mean(rec_likelihood_edge).data.item()
		results["treatment_balancing"] = torch.mean(treatment_balancing_loss).data.item()
		results["interference_balancing"] = torch.mean(interference_balancing_loss).data.item()
		results["MSE"] = torch.mean(mse_node).data.item()

		# results["MSE"] = torch.mean(pred_node-batch_dict_decoder["data"]).data.item()
