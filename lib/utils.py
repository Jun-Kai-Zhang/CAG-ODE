...
def get_next_batch(dataloader,device):
	...
	batch_dict["time_absolute"] = data_dict["time_absolute"].to(device)
	batch_dict["treatment"] = data_dict["treatment"].to(device)
	batch_dict["interference"] = data_dict["interference"].to(device)
...
def get_next_batch_test_new(dataloader,device):
	# Make the union of all time points and perform normalization across the whole dataset
	data_dict = dataloader.__next__()
...
def visualize_data_covid(model, pred_length, condition_length, dataloader,device,args,kl_coef):
	encoder, decoder, graph, num_batch = dataloader.load_test_data(pred_length=pred_length,
	 															   condition_length=condition_length)
	...
	with torch.no_grad():
		for i in tqdm(range(num_batch)):
			batch_dict_encoder = get_next_batch_new(encoder, device)
			batch_dict_graph = get_next_batch_new(graph, device)
			batch_dict_decoder = get_next_batch_test_new(decoder, device)

			results,pred_node_truth,truth_gt = model.compute_all_losses_visual_test(batch_dict_encoder, batch_dict_decoder, batch_dict_graph, args.num_atoms,
											   edge_lamda=args.edge_lamda, kl_coef=kl_coef, istest=False)
			np.save("Pre_" + str(pred_length) + "_test_pred_" + str(i) + ".npy",pred_node_truth)
			np.save("Pre_" + str(pred_length) + "_test_gt_" + str(i) + ".npy", truth_gt)
